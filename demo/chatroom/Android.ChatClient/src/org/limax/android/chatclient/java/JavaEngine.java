...
public class JavaEngine implements LimaxEngine {
	...
	public void startLogin(String username, String token, String platflag,
		...
		Endpoint.start(
				Endpoint.createLoginConfigBuilder(serverip, port, username,
						token, platflag).executor(executor)
						.variantProviderIds(pvid_chat).build(),
				new EndpointListener() {

					@Override
					public void onTransportRemoved(Transport transport)
							throws Exception {
						engineNotify.onNotify("onTransportRemoved",
								transport.toString());
					}
					...
					public void onTransportAdded(Transport transport)
						...
						final VariantManager vm = VariantManager.getInstance(
								manager, pvid_chat);
						vm.setTemporaryViewHandler("chatviews.ChatRoom",
								new TemporaryViewHandler() {

									@Override
									public void onOpen(VariantView view,
									...
									@Override
									public void onDetach(VariantView view,
											long sessionid, int reason) {
										engineNotify.onNotify(
												"onChatRoomDetach", ""
														+ sessionid + ","
														+ reason);
									}
									...
									@Override
									public void onAttach(VariantView arg0,
											long sessionid) {
										engineNotify.onNotify(
												"onChatRoomAttach",
						...
						engineNotify.onNotify("onTransportAdded",
								transport.toString());
					}

					@Override
					public void onManagerUninitialized(Manager manager) {
						engineNotify.onNotify("onManagerUninitialized",
								manager.toString());
					...
					public void onManagerInitialized(Manager manager, Config cfg) {
						...
						engineNotify.onNotify("onManagerInitialized",
								manager.toString());
					}
					...
					public void onKeepAlived(int time) {
						engineNotify.onNotify("onKeepAlived",
								Integer.toString(time));
					}

					@Override
					public void onErrorOccured(int source, int code, Throwable e) {
						String info = "source = " + source + " code = " + code
	...
	private VariantView getVariantViewByName(String viewname) {
		if (viewname.equals(chatRoomViewName))
			return chatroomview;
		final VariantManager vm = VariantManager
				.getInstance(manager, pvid_chat);
	...
	@Override
	public Runnable registerNotify(final String view, String field,
