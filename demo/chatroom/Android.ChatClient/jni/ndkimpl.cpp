...
void Java_org_limax_android_chatclient_ndk_LimaxInterface_startLogin(
	...
	auto config =
			limax::Endpoint::createLoginConfigBuilder(serverip, port, username,
					token, platflag)->variantProviderIds( { pvid_chat })->executor(
					limax::runOnUiThread)->build();
...
jstring Java_org_limax_android_chatclient_ndk_LimaxInterface_getFieldValue(
	...
	outputDebugInfo( "getFieldValue %s %s %s", view.c_str(), field.c_str(), str.c_str());
