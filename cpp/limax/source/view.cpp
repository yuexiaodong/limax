...
namespace limax {
	...
	View::View(std::shared_ptr<helper::AbstractViewContext> vc)
		: viewContext(vc), m_listeners(new helper::ViewChangedListenerContainer(mutex))
	{
	...
	int32_t View::getProviderId() const
	{
		return viewContext->getProviderId();
	}
...
namespace limax {
	...
	void View::Control::send(const View* view)
	{
		...
		protocol.providerid = view->getProviderId();
...
namespace limax {
	...
	TemporaryView::TemporaryView(std::shared_ptr<helper::AbstractViewContext> vc)
		: View(vc)
	{}
