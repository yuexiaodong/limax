...
namespace limax.endpoint
{
    ...
    public sealed class AuanyService
    {
        ...
        private class CredentialContext
        {
            ...
            private void execute(Func<string, Action> action, Result r)
            {
                try
                {
                    ...
                    EndpointConfig config = Endpoint.createLoginConfigBuilder(switcher.get("host").ToString(), switcher.get("port").intValue(), code, "", "invite").build();
            ...
            public void bind(string authcode, string username, string token, string platflag, Result r)
            {
                execute(code => () => AuanyService.bind(code, authcode, username, token, platflag, timeout, result.get(), manager), r);
            }
            ...
            public void temporary(string username, string token, string platflag, string authcode, long millisecond, byte usage, string subid, Result r)
            {
                execute(codec => () => AuanyService.temporary(username, token, platflag, appid, authcode, millisecond, usage, subid, timeout, result.get(), manager), r);
            }
            public void transfer(string username, string token, string platflag, string authcode, string temp, string authtemp, Result r)
            {
                execute(code => () => AuanyService.transfer(username, token, platflag, authcode, temp, authtemp, timeout, result.get(), manager), r);
            }
        }
        ...
        public static void bind(ServiceInfo service, string authcode, string username, string token, string platflag, long timeout, Result onresult)
        {
            ServiceInfo.SwitcherConfig switcher = service.randomSwitcherConfig();
            bind(switcher.host, switcher.port, service.appid, authcode, username, token, platflag, timeout, onresult);
        }
        public static void bind(string httpHost, int httpPort, int appid, string authcode, string username, string token, string platflag, long timeout, Result onresult)
        {
            new CredentialContext(httpHost, httpPort, appid, timeout).bind(authcode, username, token, platflag, onresult);
        }
        public static void bind(string credential, string authcode, string username, string token, string platflag, long timeout, Result onresult, EndpointManager manager)
        {
            Service.getInstance(manager).Bind(new AuanyService(onresult, timeout, ((EndpointManagerImpl)manager).dispatch).sn, credential, authcode, username, token, platflag);
        }
        public static void bind(string credential, string authcode, string username, string token, string platflag, long timeout, Result onresult)
        {
            bind(credential, authcode, username, token, platflag, timeout, onresult, Endpoint.getDefaultEndpointManager());
        }
        public static void temporary(ServiceInfo service, string credential, string authcode, string authcode2, long millisecond, byte usage, string subid, long timeout, Result onresult)
        {
            ServiceInfo.SwitcherConfig switcher = service.randomSwitcherConfig();
            temporary(switcher.host, switcher.port, service.appid, credential, authcode, authcode2, millisecond, usage, subid, timeout, onresult);
        }
        ...
        public static void temporary(ServiceInfo service, string username, string token, string platflag, string authcode, long millisecond, byte usage, string subid, long timeout, Result onresult)
        {
            ServiceInfo.SwitcherConfig switcher = service.randomSwitcherConfig();
            temporary(switcher.host, switcher.port, service.appid, username, token, platflag, authcode, millisecond, usage, subid, timeout, onresult);
        }
        public static void temporary(string httpHost, int httpPort, int appid, string username, string token, string platflag, string authcode, long millisecond, byte usage, string subid, long timeout, Result onresult)
        {
            new CredentialContext(httpHost, httpPort, appid, timeout).temporary(username, token, platflag, authcode, millisecond, usage, subid, onresult);
        }
        public static void temporary(string username, string token, string platflag, int appid, string authcode, long millisecond, byte usage, string subid, long timeout, Result onresult, EndpointManager manager)
        {
            Service.getInstance(manager).TemporaryFromLogin(new AuanyService(onresult, timeout, ((EndpointManagerImpl)manager).dispatch).sn, username, token, platflag, appid, authcode, millisecond, usage, subid);
        }
        public static void temporary(string username, string token, string platflag, int appid, string authcode, long millisecond, byte usage, string subid, long timeout, Result onresult)
        {
        ...
        public static void transfer(ServiceInfo service, string username, string token, string platflag, string authcode, string temp, string authtemp, long timeout, Result onresult)
        {
            ServiceInfo.SwitcherConfig switcher = service.randomSwitcherConfig();
            transfer(switcher.host, switcher.port, service.appid, username, token, platflag, authcode, temp, authtemp, timeout, onresult);
        }
        public static void transfer(string httpHost, int httpPort, int appid, string username, string token, string platflag, string authcode, string temp, string authtemp, long timeout, Result onresult)
        {
            new CredentialContext(httpHost, httpPort, appid, timeout).transfer(username, token, platflag, authcode, temp, authtemp, onresult);
        }
        private static void transfer(string username, string token, string platflag, string authcode, string temp, string authtemp, long timeout, Result onresult, EndpointManager manager)
        {
            Service.getInstance(manager).Transfer(new AuanyService(onresult, timeout, ((EndpointManagerImpl)manager).dispatch).sn, username, token, platflag, authcode, temp, authtemp);
        }
    ...
    public static class Endpoint
    {
        ...
        public static EndpointConfigBuilder createLoginConfigBuilder(string serverIp, int serverPort, string username, string token, string platflag)
        {
            EndpointConfigBuilderImpl config = new EndpointConfigBuilderImpl(serverIp, serverPort, platflag, username, token, false);
            return config.isConfigCanLogin() ? config : null;
        }
        public static EndpointConfigBuilder createLoginConfigBuilder(ServiceInfo service, string username, string token, string platflag)
        {
            ServiceInfo.SwitcherConfig switcher = service.randomSwitcherConfig();
        ...
        public static Closeable start(string host, int port, string username, string token, string platflag, ScriptEngineHandle handle, Executor executor)
        {
    ...
    internal class EndpointConfigBuilderImpl : EndpointConfigBuilder
    {
        ...
        internal readonly string platflag;
        ...
        public EndpointConfigBuilderImpl(string serverIp, int serverPort, string platflag,
                string username, string token, bool pingServerOnly)
        {
        ...
        internal bool isConfigCanLogin()
        {
            if (platflag.Length == 0)
                return false;
    ...
    internal class EndpointManagerImpl : EndpointManager, ClientListener, SupportDispatch
    {
        ...
        internal void process(SHandShake p)
        {
            ...
            protocol.username = config.getUserName();
            protocol.token = config.getToken();
            protocol.platflag = config.getPlatFlag();
            ScriptEngineHandle seh = config.getScriptEngineHandle();
            if (seh != null)
            {
                string dictionaryKeys = string.Join(",", seh.getDictionaryKeys());
        ...
        public Action registerListener(string fieldname, ViewChangedListener listener)
        {
            lock (this)
            {
                if (!getFieldNames().Contains(fieldname))
                    throw new Exception("View \"" + GetType().Name + "\" has no field named \"" + fieldname + "\"");
    ...
    class WebSocketConnector : ScriptSender, Closeable
    {
        ...
        public WebSocketConnector(string host, int port, string username, string token, string platflag, ScriptEngineHandle handle, Executor executor)
        {
            string pvids = String.Join(",", handle.getProviders());
            string keys = String.Join(",", handle.getDictionaryKeys());
            if (keys.Length > 0)
                keys = ";" + keys;
            this.query = "/?username=" + username + "&token=" + token + "&platflag=" + platflag + keys + "&pvids=" + pvids;
            this.handle = handle;
            this.executor = executor;
            this.stage = 0;
            handle.registerScriptSender(this);
