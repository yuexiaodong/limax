...
class ProviderManagerImpl extends AbstractRpcContext
	...
	ProviderManagerImpl(ProviderManagerConfig config, ProviderListener listener, Manager wrapper) throws Exception {
		...
		boolean tunnelEnabled = listener instanceof TunnelSupport;
		this.viewcontext = new ViewContextImpl(config, config.getViewManagerClassName()) {
			...
			@Override
			void tunnel(long sessionid, int label, Octets data) {
				if (!tunnelEnabled) {
					if (Trace.isWarnEnabled())
						Trace.warn("tunnel is not enabled");
					return;
				}
				try {
					...
					if (Trace.isErrorEnabled())
						Trace.error("tunnel sessionid = " + sessionid + " label = " + label + " data = "
								+ Helper.toHexString(data.getBytes()), e);
				}
			}
