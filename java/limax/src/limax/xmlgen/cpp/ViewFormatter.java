...
public class ViewFormatter {
		...
		if (isTemp) {
			...
			if (!new File(outputSrc, view.getFullName() + ".h").isFile()) {
				try (final PrintStream ps = FileOperation.fopen(outputSrc, view.getFullName() + ".h")) {
					...
					ps.println("	{");
					ps.println("	public:");
					ps.println("		" + name + "(std::shared_ptr<limax::helper::AbstractViewContext> vc)");
	...
	private String getBindTypeName(Bind bind, boolean istempview) {
		if (bind.isFullBind())
			return TypeName.getName(bind.getTable().getValueType());
		if (istempview)
		...
		(bind.isFullBind() ? ((Xbean) bind.getTable().getValueType()).getVariables() : bind.getVariables())
				.forEach(var -> {
					...
					if (type instanceof TypeMap) {
						...
						ps.println(
								"								" + TypeName.getName(removestype) + " __removes__;");
						...
						ps.println("							{");
						ps.println(
								"								" + TypeName.getName(vectortype) + " __newvalue__;");
	...
	public void printDefine(PrintStream ps) {
		...
		if (Main.ostream)
			Ostream.make(view, ps, "		", null);
		if (Main.cxxTrace)
			Trace.make(view, ps, "		", null);
		ps.println("	protected:");
		...
		if (hasVariables()) {
			...
			view.getBinds().forEach(bind -> {
				...
				ps.println("				{");
				if (bind.isFullBind())
					bind.getTable().getValueType()
							.accept(new Unmarshal("this->" + bind.getName(), ps, "					"));
				else
			...
			view.getSubscribes().forEach(subs -> {
				...
				if (var != null) {
					...
					if (bind.isFullBind()) {
						bind.getTable().getValueType().accept(new Unmarshal("__v__", ps, "					"));
					} else {
		...
		if (isTemp) {
			...
			ps.println("		{");
			ps.println("			if(auto vc = manager->getViewContext(" + view.getPvid()
					+ ", limax::ViewContext::Type::Static))");
