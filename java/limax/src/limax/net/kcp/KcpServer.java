...
public abstract class KcpServer implements Output, KcpListerner
{
  ...
  public void send(ByteBuf bb, KcpOnUdp ku)
  {
    ...
    private static int recv_counter = 0;
   ...
   */
	private void onReceive(DatagramPacket dp) {
		...
		if( now - countertime >= 1000){
			System.out.println("t_name :" +Thread.currentThread().getName() + "-recv_counter: " +  recv_counter);
			recv_counter = 0;
			countertime = now;
			lasttime = now;
		}else{
