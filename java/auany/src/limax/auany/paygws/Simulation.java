...
public final class Simulation implements PayGateway, HttpHandler {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		try {
			PayOrder.ok(URLDecoder.decode(uri.getQuery(), "UTF-8"), gateway);
	...
	public void onPay(long sessionid, int gateway, int payid, int product, int price, int quantity, String receipt,
		...
		String order = PayOrder.create(sessionid, gateway, payid, product, price, quantity);
		onresult.apply(ErrorSource.LIMAX, ErrorCodes.SUCCEED, order);
		AtomicReference<Future<?>> af = new AtomicReference<>();
		af.set(Engine.getProtocolScheduler().scheduleWithFixedDelay(() -> {
			af.get().cancel(false);
			try {
				PayOrder o = PayOrder.get(order);
				if (o == null) {
					if (Trace.isErrorEnabled())
						Trace.error("Simulation.proxy miss order = " + order);
					return;
				}
				if (o.getSessionId() == sessionid && o.getPayId() == payid && o.getProduct() == o.getProduct()
					...
					if (amount > 0 && amount < maxAmount)
						PayOrder.ok(order, gateway);
					else
						PayOrder.fail(order, gateway, "amount = " + amount + " but maxAmount = " + maxAmount);
				} else
					PayOrder.fail(order, gateway, "check error");
			} catch (Exception e) {
				if (Trace.isErrorEnabled())
