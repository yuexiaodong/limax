				...
				if (ctx.textAlign == 'end' || ctx.textAlign == 'right'){
					...
					for (var i=0; i<this.children.length; i++) {
						...
						if (child.attribute('x').hasValue()) {
							...
							if (i > 0){
								for(var lasti=i-1; lasti>=lastxi; lasti--){
									var c = this.children[lasti];
									c.x = lastx;
									lastx -= c.measureText(ctx);
									if (c.attribute('dx').hasValue()) lastx -= c.attribute('dx').Length.toPixels('dx');
								}
							}
							
							lastx = child.attribute('x').Length.toPixels('x');
							lastxi = i;
						}
					}
