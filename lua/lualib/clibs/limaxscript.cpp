...
static std::string tostring36(int64_t n)
{
...
#define THROW_SCRIPT_EXCEPTION throw ScriptException(__LINE__)
		...
		struct Context
		{
			...
			Context(limax::DictionaryCache *_cache) : parser(new Parser()), cache(_cache) { }
				...
				for (uint32_t i = 0; i < pl->value.size(); i += 3)
				{
					...
					if (cache)
					{
						...
						else
						{
							...
							cv.clear();
							cv.push_back('S');
							cv.append(tostring36(join.size() - 1));
							cv.push_back(':');
...
int Limax(lua_State* L)
{
	...
	lua_newtable(L);
	lua_pushlightuserdata(L, new limax::script::Context(cache));
	lua_rawsetp(L, -2, L);
	lua_pushstring(L, "__gc");
	lua_pushcfunction(L, [](lua_State *L){
		lua_getmetatable(L, 1);
		lua_rawgetp(L, -1, L);
		delete (limax::script::Context *)lua_touserdata(L, -1);
		return 0;
	});
	lua_rawset(L, -3);
			...
			if (t == 0)
			{
				...
				lua_pushcclosure(L, [](lua_State *L){
					...
					if (!ctx->z)
					{
						...
						lua_pushstring(L, "__p__");
						lua_rawget(L, 1);
						std::string __p__ = tostring36(lua_tointeger(L, -1));
						lua_pop(L, 1);
						lua_pushstring(L, "__c__");
						lua_rawget(L, 1);
						std::string __c__ = tostring36(lua_tointeger(L, -1));
						lua_pop(L, 1);
						lua_pushstring(L, "__i__");
						lua_rawget(L, 1);
						std::string __i__ = tostring36(lua_tointeger(L, -1));
						lua_pop(L, 1);
