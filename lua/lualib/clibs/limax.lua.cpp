...
namespace limax {
	...
	namespace helper {
		...
		public:
			bool initialize(lua_State* L, int table, bool fastscript, std::vector<int> providers, DictionaryCachePtr cache, ScriptErrorCollector sec)
			{
				this->sec = sec;
...
namespace limax {
	...
	{
		return createScriptEngineHandle(L, table, fastscript, providers, DictionaryCachePtr(), sec);
	}
	...
	ScriptEngineHandlePtr LuaCreator::createScriptEngineHandle(lua_State* L, int table, bool fastscript, std::vector<int> providers, DictionaryCachePtr cache, ScriptErrorCollector sec)
	{
		...
		return handle->initialize(L, table, fastscript, providers, cache, sec) ? handle : nullptr;
	}

	ScriptEngineHandlePtr LuaCreator::createScriptEngineHandle(lua_State* L, int table, bool fastscript, ScriptErrorCollector sec)
	{
		return createScriptEngineHandle(L, table, fastscript, std::vector<int>(), DictionaryCachePtr(), sec);
	}
	...
	{
		return createScriptEngineHandle(L, table, fastscript, std::vector<int>(), cache, sec);
	}
