...
int lua_create_context_and_start_endpoint(lua_State* L)
{
	...
	else
	{
		...
		builder = limax::Endpoint::createLoginConfigBuilder(serverip, serverport, username, token, paltflag);
...
int lua_auservice_bind(lua_State* L)
{
	...
	if (7 == top)
	{
		...
		limax::AuanyService::bind(credential, authcode, username, token, platflag, timeout, onresult);
	}
	else if (9 == top)
	{
		std::string httpHost = lua_check_string(L, 1);
		...
		limax::AuanyService::bind(httpHost, httpPort, appid, authcode, username, token, platflag, timeout, onresult);
	}
	else
	{
		lua_error(L);
...
int lua_auservice_temporary(lua_State* L)
{
	...
	else if (12 == top)
	{
		...
		limax::AuanyService::temporary(httpHost, httpPort, appid, username, token, platflag, authcode, millisecond, usage, subid, timeout, onresult);
	}
	else if (10 == top)
	{
		std::string username = lua_check_string(L, 1);
		...
		limax::AuanyService::temporary(username, token, platflag, appid, authcode, millisecond, usage, subid, timeout, onresult);
	}
	else
	{
		lua_error(L);
