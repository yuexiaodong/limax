return function (initializer, cache)
  ...
  local function update(s)
    ...
    if type(v) == 'string' then
		if type(c.ontunnel) == 'function' then
			c.ontunnel(p(), p(), v);
		else
			onerror("ontunnel not defined on context")
		end 
    else
		local r = t[v][p(d[v])]
		local i = p()
